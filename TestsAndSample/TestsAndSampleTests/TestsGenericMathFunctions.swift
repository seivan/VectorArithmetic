...
class TestsGenericMathFunctions<T:VectorArithmetic>:TestMathFunctions {
  ...
  func testNormalized() {
    ...
    XCTAssertEqualWithAccuracy(candidate.horizontal, expected, 0.0000001)
    XCTAssertEqualWithAccuracy(candidate.vertical, expected, 0.0000001)
    
  }
  func testLimited() {
    let candidate:T = self.vector.limited(0.5)
    var expected = 0.353553384542465
    XCTAssertEqualWithAccuracy(candidate.horizontal, expected, 0.00000001)
    XCTAssertEqualWithAccuracy(candidate.vertical, expected, 0.00000001)
  ...
  func testAngled() {
    ...
    XCTAssertEqualWithAccuracy(candidate.horizontal, expectedX, 0.00001)
    XCTAssertEqualWithAccuracy(candidate.vertical, expectedY, 0.000001)
